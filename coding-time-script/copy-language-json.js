...
function deepMerge(sourceObj, targetObj) {
  ...
  Object.entries(sourceObj).forEach(([key, value]) => {
    ...
    newObj[key] = targetObj?.[key] == null ? "TBT: " + value : targetObj[key]
  })
  return newObj
}
...
async function main() {
  const languagesWithDots = await fs.readdir(FoldersPath)
  const languages = languagesWithDots.filter((lang) => !lang.startsWith("."))
